...
@section('content')
    ...
    <div class="bd-callout" id="row_danger" style="border-left-color: red; display: none;">
        <i class="bi bi-info-circle"></i>&nbsp;&nbsp;<strong>Sorry!</strong> All four rows have been created, you cannot
        create anymore
    </div>

    <div class="card">
        <h3 class="card-header">Add bin(s) inside row {{ $row_code }}</h3>
        <div class="card-body">
            ...
            <form action="{{ route('bin.store') }}" method="post">
                ...
                <input type="hidden" name="row_width" id="row_width" value="{{ $width }}">
                @foreach ($sum as $s)
                    <input type="hidden" name="row_width_sum" id="row_width_sum" value="{{ $s->total_row_width }}">
                @endforeach
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped " id="dynamic_field2">
                            ...
                            <tbody>
                                <?php $i = 1; ?>
                                <tr>
                                    <td><input class="form-control" name="bin_name[]" placeholder="Enter Row Name"
                                            required value="B<?php echo $i; ?>" readonly=""></td>
                                    <td>
                                        <input class="form-control height_input" name="width[]" id="width" value=""
                                            type="number">
                                    </td>
                                    <td><input class="form-control " name="height[]" id="height_input1"
                                            placeholder="Enter height" value="{{ $height }}" type="number" readonly>
                                    </td>
                                    <td><input class="form-control height" name="depth[]" id="depth" placeholder="Enter Fee"
                                            value="{{ $depth }}" autocomplete="off"
                ...
                <div class="row">
                    ...
                    <div class="col-md-10">
                        <div align="end">
                            ...
                            <button type="submit" class="btn btn-primary" name="submit" id="submit_btn">Submit</button><br>
                            <br><span></span>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if (+$('#row_width_sum').val() === +$('#row_width').val()) {
                $('#row_warning').show()
            ...
            $('#add2').click(function() {
                ...
                var width = $('#width').val();
                var height = $('#height_input1').val();
                var depth = $('#depth').val();
                var rowCount = +$('#row_width_sum').val();
                ...
                for (let index = totalRowCount; index >= totalRowCount; index--) {
                    $('#dynamic_field2').append('<tr  id="row' + index +
                        '"><td><input class="form-control" name="bin_name[]" placeholder="Enter Row Name" required value="B' +
                        index + '" readonly=""></td> <td><input id="width' + index +
                        '" class="form-control height_input" type="number"  name="width[]" placeholder="Enter Width" value="" ></td><td> <input class="form-control"  name="height[]" placeholder="Enter Height"  value="'+height+'" id="height_input' +
                        index +
                        '" readonly></td> <td>  <input class="form-control"  name="depth[]" placeholder="Enter Depth" value="' +
                    ...
                    if (index >= rowCount) {
                        $('.message').show();
                        $('#warning').html("You have reached the bin limit to this row which is " + index)
                        $('#add2').prop('disabled', true);
                    }
                    ...
                    $(document).on("change", ".height_input", function() {
                        ...
                        var bin_width = +$('#width').val()
                        if (sum > bin_width) {
                            $('#row_warning').show()
                            $('.message').show();
                            $('#height_sum').html("Total width given to bins " + sum)
                            $('#submit_btn').prop('disabled', true)
                            $('#add2').prop('disabled', true)
                        } else {
